...
program
  ...
  .action((command) => {
    main({
      ...
      lib     : command.lib ?
        command.lib.map(lib => path.join((process.cwd().replace(command.root || config.root, '')), lib).replace(/\\/g, '/')) :
        false,
      imp     : command.import ?
        command.import.map(imp => path.join((process.cwd().replace(command.root || config.root, '')), imp).replace(/\\/g, '/')) :
        false,
